...
def book_histograms():
    ...
    if args.full or args.all :
        ...
        h['hist_tot'] = ROOT.TH1F("NMasked_tot", "Number of Masked Modules Overtime. Start: "+args.starttime+"; minutes ; N Modules", len(nMasked)-1 , 0, len(nMasked)-1)

    if args.bpix or args.all :

        h['hist_lyr1'] = ROOT.TH1F("NMasked_lyr1", "Number of Masked Modules Overtime (BPix). Start: "+args.starttime+"; minutes ; N Modules", len(nMasked)-1 , 0, len(nMasked)-1)
        h['hist_lyr2'] = ROOT.TH1F("NMasked_lyr2", "Number of Masked Modules Overtime (LYR2). Start: "+args.starttime+"; minutes ; N Modules", len(nMasked)-1 , 0, len(nMasked)-1)
        h['hist_lyr3'] = ROOT.TH1F("NMasked_lyr3", "Number of Masked Modules Overtime (LYR3). Start: "+args.starttime+"; minutes ; N Modules", len(nMasked)-1 , 0, len(nMasked)-1)
        h['hist_lyr4'] = ROOT.TH1F("NMasked_lyr4", "Number of Masked Modules Overtime (LYR4). Start: "+args.starttime+"; minutes ; N Modules", len(nMasked)-1 , 0, len(nMasked)-1)

    if args.fpix or args.all :

        h['hist_d1'] = ROOT.TH1F("NMasked_d1", "Number of Masked Modules Overtime (FPix). Start: "+args.starttime+"; minutes ; N Modules", len(nMasked)-1 , 0, len(nMasked)-1)
        h['hist_d2'] = ROOT.TH1F("NMasked_d2", "Number of Masked Modules Overtime (D2). Start: "+args.starttime+"; minutes ; N Modules", len(nMasked)-1 , 0, len(nMasked)-1)
        h['hist_d3'] = ROOT.TH1F("NMasked_d3", "Number of Masked Modules Overtime (D3). Start: "+args.starttime+"; minutes ; N Modules", len(nMasked)-1 , 0, len(nMasked)-1)
...
def set_style():
    ...
    for i,key in enumerate(h.keys()):
        ...
        h[key].SetMaximum(max(hmax)+10)

def make_legend():

    ld = ROOT.TLegend(0.4,0.75,0.83,0.88)
    ld.SetLineWidth(0)
    ld.SetNColumns(4)
